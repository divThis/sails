module.exports = function(sails) {
		...
		normalizeModelDef: function (modelDef, modelID) {
			...
			modelDef.connections = util.map( modelDef.connections, function (connection) {
				if ( util.isString(connection) ) {
					connection = _lookupConnection(connection, modelID, modelDef.config);
					connection = _normalizeConnection(connection, modelID, modelDef.config);
					return connection;
				}
				if ( util.isObject(connection) ) {
					return _normalizeConnection(connection, modelID, modelDef.config);
				}
	 ...
	 * @api private
	 */
	function _normalizeConnection (connection, modelID, modelConfig) {
